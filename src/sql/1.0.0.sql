CREATE TABLE `maintainer__settings`
(
...
ALTER TABLE `maintainer__settings`
    ADD PRIMARY KEY (`setting`);

INSERT INTO `maintainer__settings` (`setting`, `value`, `description`)
...
CREATE TABLE `maintainer__columns_include`
(
...
ALTER TABLE `maintainer__columns_include`
    ADD UNIQUE (`schema`, `table`, `column`);

CREATE TABLE `maintainer__columns_exclude`
(
    `schema` VARCHAR(64) CHARACTER SET `utf8mb4` COLLATE `utf8mb4_0900_as_ci` NOT NULL COMMENT 'Schema name',
    `table`  VARCHAR(64) CHARACTER SET `utf8mb4` COLLATE `utf8mb4_0900_as_ci` NOT NULL COMMENT 'Table name',
    `column` VARCHAR(64) CHARACTER SET `utf8mb4` COLLATE `utf8mb4_0900_as_ci` NOT NULL COMMENT 'Column name'
...
ALTER TABLE `maintainer__columns_exclude`
    ADD UNIQUE (`schema`, `table`, `column`);

CREATE TABLE `maintainer__tables`
(
    ...
    `analyze_histogram`         TINYINT(1) UNSIGNED                                              NOT NULL DEFAULT 0 COMMENT 'Flag indicating whether HISTOGRAM optimization should be run when ANALYZE is suggested',
    `analyze_histogram_auto`    TINYINT(1) UNSIGNED                                              NOT NULL DEFAULT 0 COMMENT 'Flag indicating whether HISTOGRAM auto update should be enabled for the MySQL table, if available',
    `analyze_histogram_buckets` INT(4) UNSIGNED                                                  NOT NULL DEFAULT 100 COMMENT 'Number of buckets to use for HISTOGRAM optimization in MySQL',
